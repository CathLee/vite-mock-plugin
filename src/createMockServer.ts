...
import type { MockMethod, ViteMockOptions } from './types'
...
function getOutputFile(filepath: string, format: string) {
...
async function resolveModule(absConfigPath: string, config: ResolvedConfig) {
  /*
 ...
 */
export async function createMockServer(opt: ViteMockOptions = { mockPath: 'mock' }, config: ResolvedConfig) {
  opt = {
    mockPath: 'mock',
    watchFiles: true,
    logger: true,
...
export async function requestMiddleware(opt: ViteMockOptions) {
  const middleware = async (req: any, res: any, next: any) => {
    ...
    if (matched) {
      ...
      const { response, timeout } = matched
